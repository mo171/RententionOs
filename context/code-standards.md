...
- Used for backend and agentic architecutre
- breakdown into controllers and services and lib and pydantic
...
- Canvas snapshots and generated specs belong in redis or anyother soluttion you recomed later in the project creation; PostgreSQL stores only the blob URL reference.
...
- `lib/` — shared infrastructure: supabase client, utilities.
- `trigger/` — all durable background tasks and AI workflows.
- `components/` — UI composition only; no business logic.
- `hooks/api/` — route handlers, triggering, and persistence.
