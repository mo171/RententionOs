...
async def update_approval_message(approval_id: str, edit: ApprovalMessageEdit):
...
async def update_approval_status(approval_id: str, update: ApprovalStatusUpdate):
    """Approve or reject an intervention — unified endpoint."""
    if update.status == "approved":
        return await approve_intervention(approval_id)
    elif update.status == "dismissed":
        return await reject_intervention(approval_id)
    else:
        raise HTTPException(status_code=400, detail="Invalid status")

@router.post("/api/approvals/{approval_id}/approve")
async def approve_intervention(approval_id: str):
    ...
    sr = row.get("strategy_result", {})
    scheduled_time = sr.get("scheduled_time")
    ...
    if draft_dict:
        try:
            ...
            if test_mode:
                print(f"[Approval] Test mode enabled. Dispatching immediately for approval {approval_id}.")
                result = send_message(draft, test_mode=test_mode)
                print(f"[Approval] Dispatch triggered for approval {approval_id}: {result}")
            else:
                print(f"[Approval] Production mode. Trigger.dev will wait until {scheduled_time} to dispatch approval {approval_id}.")
                # In production, we'd use Trigger.dev to schedule this at strategy_result.scheduled_time
                # e.g., inngest_client.send(...) or Trigger.dev webhook
        except Exception as e:
...
async def reject_intervention(approval_id: str):
